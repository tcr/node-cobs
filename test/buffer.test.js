const {encode, decode } = require('../index.js')

const { Buffer } = require('buffer');


/**
 * For consitency, Dan and I are testing the COBS encoders with the values
 * provided on the COBS wikipedia page
 * https://en.wikipedia.org/wiki/Consistent_Overhead_Byte_Stuffing#Encoding_examples
 */
const wikipediaExamples = [
  {
    id: 1,
    unencoded: [0x00],
    encoded: [0x01, 0x01],
  },
  {
    id: 2,
    unencoded: [0x00, 0x00],
    encoded: [0x01, 0x01, 0x01],
  },
  {
    id: 3,
    unencoded: [0x11, 0x22, 0x00, 0x33],
    encoded: [0x03, 0x11, 0x22, 0x02, 0x33],
  },
  {
    id: 4,
    unencoded: [0x11, 0x22, 0x33, 0x44],
    encoded: [0x05, 0x11, 0x22, 0x33, 0x44],
  },
  {
    id: 5,
    unencoded: [0x11, 0x00, 0x00, 0x00],
    encoded: [0x02, 0x11, 0x01, 0x01, 0x01],
  },
  {
    id: 6,
    unencoded: [
      ,
    ],
    encoded: [
      ,
    ],
  },
  { // 7
    id: 7,
    unencoded: [
      ,
    ],
    encoded: [
      ,

    ],
  },
  {
    id: 8,
    unencoded: [
      ,
    ],
    encoded: [
      ],
  },
  {
    id: 9,
    unencoded: [
      ],
    encoded: [
      ,
    ],
  },
  {
    id: 10,
    unencoded: [
      ,
    ],
    encoded: [
      ,
    ],
  },
];


describe('encoding', () => {
  it('should return a buffer', () => {
    const buf = Buffer.from('foo');
    const encoded = encode(buf);
    expect(Buffer.isBuffer(encoded)).toBe(true);
  });

  wikipediaExamples.map(({ id, unencoded, encoded }) => {
    it(`should encode wiki example #${id}`, () => {
      const buf = Buffer.from(unencoded);
      const cobs = encode(buf);

      expect(cobs.equals(Buffer.from(encoded))).toBe(true);
    });
  });
});

describe('decoding', () => {
  it('should return a buffer', () => {
    const buf = Buffer.from('foo');
    const encoded = encode(buf);
    const decoded = decode(encoded);
    expect(Buffer.isBuffer(decoded)).toBe(true);
  });

  wikipediaExamples.map(({ id, unencoded, encoded }) => {
    it(`should decode wiki example #${id}`, () => {
      const buf = Buffer.from(encoded);
      const cobs = decode(buf);
      const expected = Buffer.from(unencoded);

      expect(cobs.equals(expected)).toBe(true);
    });
  });
});
